...
use crate::app::App;
use crate::metrics::{CpuMetrics, DiskIoEntry, NetworkIoEntry, ProcessEntry, SystemSnapshot};
...
pub fn render(frame: &mut Frame, app: &App) {
    render_dashboard(frame, app.snapshot(), app.last_refresh_error());
}

fn render_dashboard(frame: &mut Frame, snapshot: &SystemSnapshot, refresh_error: Option<&str>) {
    let view = build_view_model(snapshot, refresh_error);
...
fn build_view_model(snapshot: &SystemSnapshot, refresh_error: Option<&str>) -> ViewModel {
    let mut status = snapshot.warnings.clone();
    if let Some(error) = refresh_error {
        status.insert(0, error.to_string());
    }
    ...
    ViewModel {
        ...
        status: (!status.is_empty()).then(|| status.join(" | ")),
    }
}

fn summary_panel_height(view: &ViewModel) -> u16 {
    let left_rows = 5 + usize::from(view.status.is_some());
...
fn render_summary(frame: &mut Frame, area: Rect, view: &ViewModel) {
    ...
    let left_row_count = 5 + usize::from(view.status.is_some());
    let left_rows = Layout::default()
        .direction(Direction::Vertical)
        .constraints(vec![Constraint::Length(1); left_row_count])
        .split(sections[0]);
    ...
    if let Some(sensors) = view.sensors.as_deref() {
        ...
        render_summary_cell(frame, left_rows[4], "", "", Style::default());
    }
...
fn truncate_text(text: &str, max_width: usize) -> String {
    ...
    let head = take_display_width(text, max_width - 3);
    format!("{head}...")
}
...
mod tests {
    ...
    use super::*;
    use crate::metrics::{
        AcceleratorMetrics, LoadAverage, MachineInfo, MemoryMetrics, RuntimeState, SensorSummary,
        UsageMetric,
    };
    ...
    #[test]
    fn truncates_text_by_terminal_display_width() {
        assert_eq!(truncate_text("处理器温度", 7), "处理...");
        assert_eq!(
            UnicodeWidthStr::width(truncate_text("处理器温度", 7).as_str()),
            7
        );
    }

    #[test]
    fn renders_diagnostics_in_a_tiny_terminal_without_panicking() {
        let snapshot = SystemSnapshot {
            machine: MachineInfo {
                machine_name: "测试机器".into(),
                kernel: "6.0".into(),
                arch: "aarch64".into(),
            },
            ...
            memory: MemoryMetrics {
                ram: UsageMetric::from_total_and_used(1024, 512),
                swap: UsageMetric::from_total_and_used(0, 0),
                zram: UsageMetric::from_total_and_used(0, 0),
            },
            process_list: Vec::new(),
            uptime: Duration::from_secs(60),
            overall_usage_percent: 25.0,
            cpus: Vec::new(),
            network_io: Vec::new(),
            disk_io: Vec::new(),
            sensor_summary: Vec::new(),
            accelerators: AcceleratorMetrics::default(),
            warnings: vec!["hwmon unavailable".into()],
        };
        let backend = TestBackend::new(20, 6);
        let mut terminal = Terminal::new(backend).unwrap();

        terminal
            .draw(|frame| render_dashboard(frame, &snapshot, Some("refresh failed")))
            .unwrap();
    }
